...
-- MAGIC 
-- MAGIC ## assert is like a try catch, should have 2252 records
...
-- going to look like the frirst two lines of cmd line 4 create table and key
CREATE TABLE IF NOT EXISTS events_raw
  (key BINARY, offset BIGINT, partition INT, timestamp BIGINT, topic STRING, value BINARY)

-- table a and table b; want to insert data from a into b so we use INSERT INTO table a from table b --> use a select * and create table as a select --> use sql statement and that will use 

-- create table and want to insert at the same tim eis using sql 
-- ex:  insert into events_raw 
-- select * from events_json

-- specifies location because we are pointing it to somewhere 
-- create database table = it by default is unmanaged
-- pullin gsource data = unmanaged data and creating a JSON parque table 
